 ...
 *
 * Принимает один файл изображения и переключает профиль на кастомный аватар.
 */
export const uploadSettingsAvatarMutation = (options?: Partial<Options<UploadSettingsAvatarData>>): UseMutationOptions<UploadSettingsAvatarResponse, UploadSettingsAvatarError, Options<UploadSettingsAvatarData>> => {
    const mutationOptions: UseMutationOptions<UploadSettingsAvatarResponse, UploadSettingsAvatarError, Options<UploadSettingsAvatarData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await uploadSettingsAvatar({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
